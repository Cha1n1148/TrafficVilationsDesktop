...
namespace TrafficVilationsDesktop.View.ViolationTypePages
{
    ...
    public partial class MainPage : Page
    {
        ...
        public MainPage(NavigationContext navigationContext)
        {
            ...
            DataGrid_Main.ItemsSource = _dbContext.Violation_Types.Local.ToBindingList();
        }
        ...
        {
            var record = DataGrid_Main.SelectedItem as Violation_Types;
            if (record == null)
            {
        ...
        private void Delete_Button_Click(object sender, RoutedEventArgs e)
        {
            ...
            else
            {
                if (MessageBox.Show("Вы уверены?", "Удаление записи.", MessageBoxButton.OKCancel) == MessageBoxResult.OK)
                {
                    try
                    {
                        _dbContext.Violation_Types.Remove(record);
